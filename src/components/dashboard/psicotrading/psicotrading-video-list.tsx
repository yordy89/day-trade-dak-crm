...
const PsicoTradingVideoList: React.FC = () => {
  const {
    data: videos,
    isLoading,
    error,
    ...
    queryFn: async () => {
      const response = await API.get('/videos/psicotradingVideos');
      return response.data;
    },
  });

  if (isLoading) return <CircularProgress sx={{ display: 'block', mx: 'auto', mt: 5 }} />;
  if (error) return <Typography color="error">Error fetching PsicoTrading videos.</Typography>;
    ...
    >
      {videos?.map((video: StockVideo) => (
        <Box key={video.key} sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          {video.key.replace('PsicoTrading/', '').replace('.mp4', '')}
          </Typography>
        </Box>
      ))}
    </Box>
...
export default PsicoTradingVideoList;
